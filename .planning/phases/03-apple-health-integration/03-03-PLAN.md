...
must_haves:
  ...
  key_links:
    - from: "client/src/components/meatspace/tabs/HealthTab.jsx"
      to: "/api/apple-health/metrics/:metricName/daily"
      via: "api.getAppleHealthMetrics()"
      pattern: "getAppleHealthMetrics"
    - from: "client/src/components/meatspace/tabs/HealthTab.jsx"
      to: "/api/apple-health/correlation"
...
API response shapes:
```javascript
// GET /api/apple-health/metrics/:metricName/daily
[{ date: "2024-01-15", value: 8432 }, ...]

// GET /api/apple-health/metrics/:metricName
{ metricName, count, latest, average, from, to, dataPoints: [...] }

// GET /api/apple-health/correlation
...
- Import correlation components (AlcoholHrvCorrelation, ActivityBloodCorrelation) — these will be created in Task 2. For now, conditionally render them only if `correlationData` exists. Pass `correlationData` and `range` as props.
  </action>
  <verify>
...
**3. Add Health tab to constants.js:**
...
**4. Wire Health tab into MeatSpace.jsx:**
...
5. URL is deep-linkable
  ...
  <verify>
    <automated>curl -s http://localhost:5554/api/apple-health/range | node -e "process.stdin.on('data',d=>{console.log('PASS: range endpoint responding')})"</automated>
  </verify>
