...
- **15 curated health markers**: Pre-built database covering longevity (FOXO3A, IGF1R, CETP, IPMK), tumor suppression (TP53), iron metabolism (HFE H63D, C282Y), methylation (MTHFR C677T/A1298C, COMT, AHCY), caffeine sensitivity (ADA), detoxification (CYP2B6, SOD2), and inflammation (IL-6)
...
- **Category-grouped display**: Markers displayed in collapsible cards grouped by health category with color-coded status badges, expandable details, implications, and editable personal notes
...
**Server changes**:
- `server/lib/curatedGenomeMarkers.js` — Curated SNP database with 15 markers, category config, `classifyGenotype()` and `formatGenotype()` functions
...
- `server/routes/genome.js` — 8 REST endpoints mounted at `/api/digital-twin/genome`
