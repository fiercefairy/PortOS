...
export default function CityAgentBar({ cosAgents, agentMap }) {
  ...
  return (
    <div className="absolute bottom-3 left-3 right-3 pointer-events-auto">
      <div className="bg-black/70 backdrop-blur-sm border border-cyan-500/20 rounded-lg px-3 py-2">
        <div className="flex items-center gap-3 overflow-x-auto">
          <span className="text-cyan-400 text-[10px] font-pixel tracking-wider font-bold shrink-0">AGENTS</span>
          {activeAgents.map((agent, i) => {
            ...
            return (
              <div
                key={agent.agentId || i}
                className="flex items-center gap-1.5 px-2 py-1 bg-black/40 border border-white/10 rounded text-[9px] font-pixel tracking-wide shrink-0"
              >
                <span
