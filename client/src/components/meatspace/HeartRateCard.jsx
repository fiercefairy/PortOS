...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', color: '#fff', padding: '8px', borderRadius: '6px', fontSize: '12px' }}>
      ...
      <p style={{ color: '#ef4444', fontWeight: 600 }}>{Math.round(payload[0].value)} bpm</p>
    </div>
  );
};

export default function HeartRateCard({ data = [], loading = false }) {
  ...
  return (
    <div className="bg-port-card border border-port-border rounded-xl p-6">
      <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">Heart Rate</h3>
      {loading ? (
        <div className="flex items-center justify-center h-40 text-gray-600 text-sm">Loading...</div>
      ) : avg == null ? (
        <div className="flex items-center justify-center h-40 text-gray-600 text-sm">No heart rate data available</div>
      ) : (
        <>
          <div className="mb-4">
